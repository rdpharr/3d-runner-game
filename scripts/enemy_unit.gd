...
func _ready() -> void:
	# Setup sprite
	sprite.texture = load("res://assets/pixellab/enemy.png")
	...
	collision_layer = 8  # Enemy unit layer
	collision_mask = 4   # Player unit layer
...
func set_collision_active(active: bool) -> void:
	"""Enable/disable collision monitoring based on proximity to player"""
	if not is_dying:
		set_deferred("monitoring", active)
		set_deferred("monitorable", active)

func _on_area_entered(area: Area2D) -> void:
	"""Handle collision with player unit"""
	if is_dying:
		return

	if area is PlayerUnit and not area.is_dying:
